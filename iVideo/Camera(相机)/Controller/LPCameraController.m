...
@interface LPCameraController () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate, AVCaptureMetadataOutputObjectsDelegate>
...
@property (nonatomic, strong) AVCaptureVideoDataOutput *videoOutput;   // video data output
@property (nonatomic, strong) AVCaptureAudioDataOutput *audioOutput;   // audio data output
...
@property (nonatomic, strong) LPMovieWriter *movieWriter;
...
- (BOOL)setupSessionOutputsWithError:(NSError **)error {
    ...
    if ([self.session canAddOutput:self.audioOutput]) {
        ...
        NSDictionary *info = @{NSLocalizedDescriptionKey : @"Failed to add audio output!"};
        *error = [NSError errorWithDomain:LPCameraErrorDomain
                                     code:LPCameraErrorCodeFailedToAddOutput
                                 userInfo:info];
    }
    ...
    self.movieWriter = [[LPMovieWriter alloc] initWithVideoSettings:videoSettings audioSettings:audioSettings dispatchQueue:self.videoQueue];
    ...
    if ([self.session canAddOutput:self.metadataOutput]) {
        ...
        NSDictionary *info = @{NSLocalizedDescriptionKey : @"Failed to add face output!"};
        *error = [NSError errorWithDomain:LPCameraErrorDomain
                                     code:LPCameraErrorCodeFailedToAddOutput
                                 userInfo:info];
    }
...
- (void)stopRecording {
    [self.movieWriter stopWritingWithCompletion:^(NSURL *outputURL) {
        [self.library writeVideoAtURL:outputURL success:^(UIImage *image) {
            [self postThumbnailNotificationWithImage:image];
        } failure:^(NSError *error) {
            dispatch_async(MAIN_QUEUE, ^{
...
#pragma mark - capture video/audio data output sample buffer delegate
- (void)captureOutput:(AVCaptureOutput *)captureOutput
didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer
    ...
    if (captureOutput == self.videoOutput) {
        ...
        CIImage *srcImage = [CIImage imageWithCVImageBuffer:imageBuffer];
        [self.imageTarget setImage:srcImage];
    }
}
